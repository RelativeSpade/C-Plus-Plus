...
void setData(
	...
	string sarrive[],
	int sseats[],
	int sseatsOnThePlane[],
	const int sSIZE) { // start setData
	ifstream inFile;
...
void findFlight(
	...
	// create an int index = -1.  We use -1 because this is not a valid index
	// value.  If the index changes to a positive value that means the target
	// was found. if index is still -1 that means the target was not found
	int index = -1;
	...
	for (int i = 0; i < fSIZE;
		 i++) { // if the two values are equal, set the index to the index value
	...
	// Now test to see if index is still -1, if it is then print the number of
	// seats available.  If it is not -1 then display the numbe of seats
	// available.
	if (index == -1) {
		cout << "No flight found from " << cdepart << " to " << carrive << endl;
	} else {
		cout << "There are " << fseats[index] << " Seats available from "
			 << cdepart << " to " << carrive << endl;
	}
...
void reserveSeat(int rflightNumber[], int ravailable[], int rSEATS) {
	// index variable to store the index where a flight number was found.
	...
	if (index == -1) {
		...
		// index is no longer -1 and contains the index of the flight to reserve
		// seats

		// delcare a variable to store the number of seats to reserve
		int numSeats = 0;

		// prompt for the number of seats to reserve and stor in the variable
		// you just declared.
		...
		if (numSeats <= ravailable[index]) {
			...
			cout << "Sorry there are only " << ravailable[index]
				 << " seats on the flight\n";
		}
	}
...
void cancelReservation(int cflightNumber[],int cseats[],int
cseatsOnThePlane[],int cSIZE)
{//start cancelReservation
  ...
  //Test to see if index is still -1 in this case the flight was not found
	 ...
	 //delcare a variable to store the number of seats to cancel
  
  //prompt for the number of seats to cancel and store the value in the variable you just declared. 
  
  cout<<"Enter the number of seats to cancel:\n";
   

  //test to see if there are enough seats reserved to cancel.
  //That is if your plane has a total of 500 seats and there are 450 available,then you can only cancel 50 seats.  If the user tries to cancel more display themessage "You can not cancel that many seats" 
 
	//add sumSeats to the number of available seats
 

	//display a message stating how many seats were reserved
