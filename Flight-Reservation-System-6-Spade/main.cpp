...
{//start cancelReservation
  ...
  if(numSeats <= cseatsOnThePlane[index]-cseats[index])
  {
    ...
    cout<<"You canceled "<<numSeats<<" seats\n";
     cout<<"There are now "<<cseats[index]<<"seats on the plan";
...
void listAllFlights(int sflightNumber[],string sdepart[],string sarrive[],int sseats[],int sseatsOnThePlane[],const int sSIZE)
{
   cout<<setw(15)<<"Flight Number:"<<setw(15)<<"Origin"<<setw(15)<<"Destination:"<<setw(15)<<"Seats Available\n";
  for (int i = 0; i< sSIZE; i++ ){
    cout<<setw(15)<<sflightNumber[i]<<setw(15)<<sdepart[i]<<setw(15)<<sarrive[i]<<setw(15)<<sseatsOnThePlane[i]<<setw(15)<<endl;
...
{ int i = -1;
  switch(eans)
  {
    ...
    case 2:
      ...
      case 5:
        listAllFlights(eflightNumber,edepart,earrive,eseats,eseatsOnThePlane,eSIZE);
      break;
